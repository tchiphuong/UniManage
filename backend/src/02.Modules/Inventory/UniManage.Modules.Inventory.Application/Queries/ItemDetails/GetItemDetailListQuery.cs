...
namespace UniManage.Modules.Inventory.Application.Queries.ItemDetails
{
    ...
    {
        public async Task<ApiResponse<PagedResult<GetItemDetailListQuery.Result>>> Handle(GetItemDetailListQuery request, CancellationToken cancellationToken)
        {
            var log = new ApiLogModel(request.HeaderInfo)
            {
                Parameter = new List<LogParamModel>
                {
                    ...
                    new LogParamModel(nameof(request.Type), request.Type?.ToString())
                }
            };
            ...
            using (var dbContext = new DbContext())
            {
                try
                {
                    ...
                    var items = await dbContext.QueryAsync<GetItemDetailListQuery.Result>(sql.ToString(), parameters, cancellationToken);
