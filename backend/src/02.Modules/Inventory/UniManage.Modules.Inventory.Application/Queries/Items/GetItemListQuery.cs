...
namespace UniManage.Modules.Inventory.Application.Queries.Items
{
    ...
    {
        public async Task<ApiResponse<PagedResult<GetItemListQuery.Result>>> Handle(GetItemListQuery request, CancellationToken cancellationToken)
        {
            var log = new ApiLogModel(request.HeaderInfo)
            {
                Parameter = new List<LogParamModel>
                {
                    ...
                    new LogParamModel(nameof(request.BrandCode), request.BrandCode),
                    new LogParamModel(nameof(request.CategoryCode), request.CategoryCode),
                    new LogParamModel(nameof(request.ColorCode), request.ColorCode),
                    new LogParamModel(nameof(request.SizeCode), request.SizeCode)
                }
            };
            ...
            using (var dbContext = new DbContext())
            {
                try
                {
                    ...
                    var items = await dbContext.QueryAsync<GetItemListQuery.Result>(sql.ToString(), parameters, cancellationToken);
