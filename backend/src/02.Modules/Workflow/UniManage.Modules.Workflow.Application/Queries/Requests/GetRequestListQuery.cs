...
namespace UniManage.Modules.Workflow.Application.Queries.Requests
{
    ...
    {
        public async Task<ApiResponse<PagedResult<GetRequestListQuery.Result>>> Handle(GetRequestListQuery request, CancellationToken cancellationToken)
        {
            var log = new ApiLogModel(request.HeaderInfo)
            {
                Parameter = new List<LogParamModel>
                {
                    ...
                    new LogParamModel(nameof(request.Status), request.Status),
                    new LogParamModel(nameof(request.ApplicantEmployeeCode), request.ApplicantEmployeeCode),
                ...
                try
                {
                    ...
                    sql.AppendLine(@"
                        SELECT 
                            ...
                            r.CreatedAt
                        FROM wf_request r
                        LEFT JOIN HrEmployees e ON r.ApplicantEmployeeCode = e.EmployeeCode
                    ...
                    var items = await dbContext.QueryAsync<GetRequestListQuery.Result>(sql.ToString(), parameters, cancellationToken);
