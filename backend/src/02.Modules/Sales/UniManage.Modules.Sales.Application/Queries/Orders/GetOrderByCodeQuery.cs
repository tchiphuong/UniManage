...
namespace UniManage.Modules.Sales.Application.Queries.Orders
{
        ...
        public async Task<ApiResponse<GetOrderByCodeQuery.Result>> Handle(GetOrderByCodeQuery request, CancellationToken cancellationToken)
        {
            ...
            using (var dbContext = new DbContext())
            {
                try
                {
                    ...
                    var items = await dbContext.QueryAsync<GetOrderByCodeQuery.OrderItemDto>(itemsSql, new { OrderCode = request.OrderCode }, cancellationToken);
