...
namespace UniManage.Modules.HumanResource.Application.Queries.EmployeeShifts
{
    ...
    {
        public async Task<ApiResponse<PagedResult<GetEmployeeShiftListQuery.Result>>> Handle(GetEmployeeShiftListQuery request, CancellationToken cancellationToken)
        {
            var log = new ApiLogModel(request.HeaderInfo)
            {
                Parameter = new List<LogParamModel>
                {
                    ...
                    new LogParamModel(nameof(request.WorkShiftCode), request.WorkShiftCode),
                    new LogParamModel(nameof(request.FromDate), request.FromDate?.ToString("yyyy-MM-dd")),
                    new LogParamModel(nameof(request.ToDate), request.ToDate?.ToString("yyyy-MM-dd"))
                }
            };
                ...
                try
                {
                    ...
                    sql.AppendLine(@"
                        SELECT 
                            ...
                            ws.EndTime,
                            es.CreatedAt
                        FROM HrEmployeeShifts es
                        INNER JOIN HrEmployees e ON es.EmployeeCode = e.EmployeeCode
                        INNER JOIN HrWorkShifts ws ON es.WorkShiftCode = ws.Code
                        WHERE 1=1");
                    ...
                    var items = await dbContext.QueryAsync<GetEmployeeShiftListQuery.Result>(sql.ToString(), parameters, cancellationToken);
