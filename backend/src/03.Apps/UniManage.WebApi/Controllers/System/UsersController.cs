...
using UniManage.Modules.System.Application.Commands.User;
using UniManage.Modules.System.Application.Queries.User;
...
public class UsersController : BaseController
{
    ...
    [HttpGet("{uuid}/roles")]
    [PermissionAuthorize(CoreFunction.SyUser, CoreAction.View)]
    public async Task<ActionResult<ApiResponse<List<UserRoleDto>>>> GetRoles(Guid uuid, CancellationToken cancellationToken)
    {
    ...
    public async Task<ActionResult<ApiResponse<bool>>> ChangePassword([FromBody] ChangePasswordCommand command, CancellationToken cancellationToken)
    {
        ...
        command.Username = username; // [SECURITY] REQUIRED: Láº¥y username tá»« Token Ä‘á»ƒ trÃ¡nh IDOR
        var result = await _mediator.Send(command, cancellationToken);
        return Ok(result);
    }
