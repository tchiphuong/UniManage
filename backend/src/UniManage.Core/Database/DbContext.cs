...
namespace UniManage.Core.Database
{
    ...
    public class DbContext : Microsoft.EntityFrameworkCore.DbContext
    {
        ...
        private static string GetConnectionString()
        {
            try
            {
                var config = UniManage.Core.Utilities.ConfigHelper.Configuration;
                ...
                foreach (var prop in type.GetProperties())
                {
                    ...
                    if (refKeyAttr != null)
                    {
                        ...
                        if (parts.Length == 2)
                        {
                            ...
                            var targetKeyName = parts[1];

                            var targetType = entityTypes.FirstOrDefault(t => t.Name == targetTableName);
                            if (targetType != null)
                            {
                                var navProp = type.GetProperties().FirstOrDefault(p => p.PropertyType == targetType);
