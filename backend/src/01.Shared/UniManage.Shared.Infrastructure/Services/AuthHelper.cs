...
namespace UniManage.Shared.Infrastructure.Services
{
    ...
    public static class AuthHelper
    {
        ...
        public static async Task<(bool Success, string? Error, SyUserSecurityModel? User)> ValidateUserStatusAsync(string username, ApiLogModel log, CancellationToken ct = default)
        {
            ...
            var userEntity = await dbContext.Set<SyUsers>()
                .AsNoTracking()
        ...
        /// <param name="log">Log object from the calling API.</param>
        /// <param name="ct">Cancellation token.</param>
        public static async Task HandleLoginSuccessAsync(string username, ApiLogModel log, CancellationToken ct = default)
        {
            log.Parameter.Add(new LogParamModel(nameof(username), username));
        ...
        /// <param name="log">Log object from the calling API.</param>
        /// <param name="ct">Cancellation token.</param>
        public static async Task HandleLoginFailureAsync(string username, ApiLogModel log, CancellationToken ct = default)
        {
            log.Parameter.Add(new LogParamModel(nameof(username), username));
            using var dbContext = new DbContext(openTransaction: true);
        ...
        /// <param name="log">Log object from the calling API.</param>
        /// <param name="ct">Cancellation token.</param>
        public static async Task UpdateDeviceTokenAsync(long userId, string? deviceId, string? fcmToken, string? deviceType, ApiLogModel log, CancellationToken ct = default)
        {
            log.Parameter.Add(new LogParamModel(nameof(userId), userId));
            log.Parameter.Add(new LogParamModel(nameof(deviceId), deviceId));
            
            if (string.IsNullOrEmpty(deviceId)) return;

            using var dbContext = new DbContext(openTransaction: true);
            var sql = @"
                IF EXISTS (SELECT 1 FROM [dbo].[SyUserDevices] WHERE [UserId] = @UserId AND [DeviceId] = @DeviceId)
                BEGIN
                    UPDATE [dbo].[SyUserDevices]
                ...
                ELSE
                BEGIN
                    INSERT INTO [dbo].[SyUserDevices] ([UserId], [DeviceId], [FcmToken], [DeviceType], [CreatedAt], [LastActiveAt])
        ...
        /// </summary>
        public static async Task RegisterBiometricKeyAsync(long userId, string deviceId, string publicKey, string? deviceName, ApiLogModel log, CancellationToken ct = default)
        {
            log.Parameter.Add(new LogParamModel(nameof(userId), userId));
            log.Parameter.Add(new LogParamModel(nameof(deviceId), deviceId));
            ...
            using var dbContext = new DbContext(openTransaction: true);
            var sql = @"
                IF EXISTS (SELECT 1 FROM [dbo].[SyUserBiometrics] WHERE [UserId] = @UserId AND [DeviceId] = @DeviceId)
                BEGIN
                    UPDATE [dbo].[SyUserBiometrics]
                ...
                ELSE
                BEGIN
                    INSERT INTO [dbo].[SyUserBiometrics] ([UserId], [DeviceId], [PublicKey], [DeviceName], [CreatedAt])
        ...
        /// </summary>
        public static async Task<bool> VerifyBiometricSignatureAsync(long userId, string deviceId, string challenge, string signature, ApiLogModel log, CancellationToken ct = default)
        {
            log.Parameter.Add(new LogParamModel(nameof(userId), userId));
            log.Parameter.Add(new LogParamModel(nameof(deviceId), deviceId));
            ...
            var publicKey = await dbContext.Set<SyUserBiometrics>()
                .AsNoTracking()
        ...
        public static async Task<SyUsers?> GetOrCreateSocialUserAsync(UniManage.Shared.Domain.Interfaces.SocialUserProfile profile, string internalSecret, ApiLogModel log, CancellationToken ct = default)
        {
            ...
            dbContext.Set<SyUsers>().Add(user);
            await dbContext.SaveChangesAsync(ct);
            await dbContext.CommitAsync(ct);
