...
namespace UniManage.Shared.Infrastructure.Pipelines
{
        ...
        where TRequest : IRequest<TResponse>
    {
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            var log = new ApiLogModel(headerInfo ?? new HeaderInfo())
            {
                Method = "Pipeline",
            ...
            log.Parameter = new List<LogParamModel> 
            { 
                new LogParamModel("RequestBody", MaskSensitiveData(rawParams)) 
            };
            ...
            catch (Exception ex)
            {
                ...
                if (responseType.IsGenericType && responseType.GetGenericTypeDefinition() == typeof(ApiResponse<>))
                {
                    ...
                    apiResponseType.GetProperty(nameof(ApiResponse<object>.Errors))!.SetValue(response, new List<FieldError> { new FieldError("", ex.Message) });
