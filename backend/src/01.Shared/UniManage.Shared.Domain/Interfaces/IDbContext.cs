...
///   {
///     var users = await db.Set&lt;SyUsers&gt;().Where(u => u.Status == "active").ToListAsync();
///   }
///   
///   // Write operations (with transaction)
///   using (var db = new DbContext(openTransaction: true))
///   {
///     var user = await db.Set&lt;SyUsers&gt;().FindAsync(userId);
...
public interface IHybridDbContext : IDbContext
{
    ...
    /// </summary>
    new Task<IEnumerable<T>> QueryAsync<T>(string sql, object? param = null, CancellationToken cancellationToken = default);

    /// <summary>
    /// Execute Dapper command on EF Core context
    /// </summary>
    new Task<int> ExecuteAsync(string sql, object? param = null, CancellationToken cancellationToken = default);

    /// <summary>
    /// Execute Dapper scalar query on EF Core context
    /// </summary>
    new Task<T> ExecuteScalarAsync<T>(string sql, object? param = null, CancellationToken cancellationToken = default);
}
