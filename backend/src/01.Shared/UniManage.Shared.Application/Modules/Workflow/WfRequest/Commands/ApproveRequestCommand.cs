namespace UniManage.Shared.Application.Modules.WfRequest.Commands
{
    ...
    {
        public async Task<ApiResponse<ApproveRequestCommand.Response>> Handle(ApproveRequestCommand request, CancellationToken cancellationToken)
        {
            ...
            using var dbContext = new DbContext(openTransaction: true);
                    ...
                    var pendingApproval = await dbContext.QueryFirstOrDefaultAsync<dynamic>(getPendingApprovalSql, new { RequestId = request.RequestId, ApproverUsername = currentUsername }, cancellationToken);
...
return errorResponse;
                    ...
                    var remainingPending = await dbContext.ExecuteScalarAsync<int>(checkLevelCompleteSql, new { ApprovalRouteLevelId = (int)pendingApproval.ApprovalRouteLevelId }, cancellationToken);
                    ...
                    if (remainingPending == 0)
                    {
                        ...
                        var nextLevelId = await dbContext.ExecuteScalarAsync<int?>(getNextLevelSql, new { CurrentLevelId = (int)pendingApproval.ApprovalRouteLevelId, CurrentLevelOrder = (int)pendingApproval.LevelOrder }, cancellationToken);
                        ...
                        if (nextLevelId.HasValue)
                        {
                            ...
                            var nextApprovers = await dbContext.QueryAsync<dynamic>(getNextApproversSql, new { LevelId = nextLevelId.Value }, cancellationToken);
