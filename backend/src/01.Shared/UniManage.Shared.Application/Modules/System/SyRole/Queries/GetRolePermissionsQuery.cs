...
namespace UniManage.Shared.Application.Modules.SyRole.Queries
{
    ...
    public sealed class GetRolePermissionsQuery : BaseQuery, IRequest<ApiResponse<List<GetRolePermissionsQuery.Result>>>
    {
        ...
        public sealed class Result
        {
            ...
            /// </summary>
            public List<ActionPermissionModel> Actions { get; set; } = new();
        }

        /// <summary>
        /// Chi tiết hành động và trạng thái gán quyền
        /// </summary>
        public sealed class ActionPermissionModel
        {
            /// <summary>
    ...
    public sealed class GetRolePermissionsQueryHandler : IRequestHandler<GetRolePermissionsQuery, ApiResponse<List<GetRolePermissionsQuery.Result>>>
    {
        ...
        public async Task<ApiResponse<List<GetRolePermissionsQuery.Result>>> Handle(GetRolePermissionsQuery request, CancellationToken cancellationToken)
        {
            ...
            using (var dbContext = new DbContext())
            {
                try
                {
                    ...
                    var functions = (await dbContext.QueryAsync<FunctionRawModel>(sqlFunctions, null, cancellationToken)).ToList();
                    ...
                    var actions = (await dbContext.QueryAsync<ActionRawModel>(sqlActions, null, cancellationToken)).ToList();
                    ...
                    var mappings = (await dbContext.QueryAsync<MappingRawModel>(sqlMappings, null, cancellationToken)).ToList();
                    ...
                    var assignedPermissions = (await dbContext.QueryAsync<PermissionRawModel>(sqlRolePermissions, new { request.RoleCode }, cancellationToken)).ToList();
                    ...
                    foreach (var func in functions)
                    {
                        ...
                        foreach (var act in actions)
                        {
                            ...
                            if (supportedActions.Contains(act.Code))
                            {
                                ...
                                row.Actions.Add(new GetRolePermissionsQuery.ActionPermissionModel
                                {
                                    ActionCode = act.Code,
                                    ActionName = actName,
        ...
        private sealed class FunctionRawModel
        {
        ...
        private sealed class ActionRawModel
        {
            public string Code { get; set; } = default!;
            public string ResourceKey { get; set; } = default!;
        }

        private sealed class MappingRawModel
        {
            public string FunctionCode { get; set; } = default!;
            public string ActionCode { get; set; } = default!;
        }

        private sealed class PermissionRawModel
        {
            public string FunctionCode { get; set; } = default!;
            public string ActionCode { get; set; } = default!;
        }
