...
namespace UniManage.Shared.Application.Modules.SyMenu.Queries;
...
public sealed class GetMenuListQueryHandler : IRequestHandler<GetMenuListQuery, ApiResponse<List<GetMenuListQuery.Response>>>
{
    public async Task<ApiResponse<List<GetMenuListQuery.Response>>> Handle(GetMenuListQuery request, CancellationToken ct)
    {
        var log = new ApiLogModel(request.HeaderInfo)
        {
            ...
            {
                new LogParamModel("Username", request.HeaderInfo?.Username ?? "Anonymous")
            }
        };
        ...
        using (var dbContext = new DbContext())
        {
            try
            {
                ...
                // L?y menus user c� quy?n truy c?p
                var userMenusSql = @"SELECT DISTINCT m.Id, m.Code, m.FunctionCode, m.ParentCode, m.ResourceKey, m.Url, m.Icon 
                           FROM SyMenus m
                           INNER JOIN SyFunctions f ON m.FunctionCode = f.Code
                           INNER JOIN SyRolePermissions rp ON f.Code = rp.FunctionCode
                           INNER JOIN SyUserRoles ur ON rp.RoleCode = ur.RoleCode
                ...
                // N?u user kh�ng c� quy?n truy c?p b?t k? menu n�o ? 403 Forbidden
                if (!userMenus.Any())
                {
                ...
                // L?y t?t c? parent codes d? quy d? d?m b?o tree structure ho�n ch?nh
                ...
                // N?u c� parent menus thi?u, l?y ch�ng t? DB (d? quy l?y t?t c? parents)
