...
namespace UniManage.Shared.Application.Modules.SaOrder.Queries
{
    ...
    {
        public async Task<ApiResponse<GetOrderByCodeQuery.Result>> Handle(GetOrderByCodeQuery request, CancellationToken cancellationToken)
        {
            var log = new ApiLogModel(request.HeaderInfo)
            {
                Parameter = new List<LogParamModel>
                {
                    new LogParamModel(nameof(request.OrderCode), request.OrderCode)
                }
            };
            ...
            using (var dbContext = new DbContext())
            {
                try
                {
                    var orderSql = @"
                        ...
                        LEFT JOIN HrEmployees e ON o.EmployeeCode = e.EmployeeCode
                        WHERE o.OrderCode = @OrderCode";

                    var order = await dbContext.QueryFirstOrDefaultAsync<GetOrderByCodeQuery.Result>(orderSql, new { OrderCode = request.OrderCode }, cancellationToken);
                    ...
                    var items = await dbContext.QueryAsync<GetOrderByCodeQuery.OrderItemModel>(itemsSql, new { OrderCode = request.OrderCode }, cancellationToken);
