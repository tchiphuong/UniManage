...
public sealed class CreateWorkShiftCommandValidator : AbstractValidator<CreateWorkShiftCommand>
{
    ...
    private static async Task<bool> IsCodeExistsAsync(string code)
    {
        ...
        {
            return await dbContext.Set<HrWorkShifts>().AnyAsync(x => x.Code == code);
        }
    }
}
...
{
    public async Task<ApiResponse<CreateWorkShiftCommand.Response>> Handle(CreateWorkShiftCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var workShift = new HrWorkShifts
            {
                Code = request.Code,
                Name = request.Name,
            ...
            dbContext.Set<HrWorkShifts>().Add(workShift);
