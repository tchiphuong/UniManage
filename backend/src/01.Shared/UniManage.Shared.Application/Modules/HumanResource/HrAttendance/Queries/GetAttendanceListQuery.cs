...
namespace UniManage.Shared.Application.Modules.HrAttendance.Queries
{
    #region Query
    ...
    {
        public async Task<ApiResponse<PagedResult<GetAttendanceListQuery.Response>>> Handle(GetAttendanceListQuery request, CancellationToken cancellationToken)
        {
            var log = new ApiLogModel(request.HeaderInfo)
            {
                Parameter = new List<LogParamModel>
                {
                    ...
                    new LogParamModel(nameof(request.FromDate), request.FromDate),
                    new LogParamModel(nameof(request.ToDate), request.ToDate),
            ...
            using (var dbContext = new DbContext())
            {
                try
                {
                    ...
                    query.AppendLine($@"SELECT 
                                    ...
                                    FROM HrAttendance a
                                    INNER JOIN HrEmployees e ON a.EmployeeCode = e.EmployeeCode
                                    WHERE 1 = 1");
