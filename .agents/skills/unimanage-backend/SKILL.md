---
name: UniManage Backend Developer
description: Comprehensive guidelines for developing the UniManage backend using .NET 9, CQRS, MediatR, Dapper, and EF Core.
---
...
**🔥 QUY TẮC MỚI (BẮT BUỘC - STRICT MODE):**
- **ĐỊNH DẠNG SQL**: Mọi câu lệnh raw SQL trong chuỗi string (@"...") **PHẢI** xuống dòng và tab canh lề thẳng hàng cho từng field. KHI NỐI CHUỖI SQL ĐỘNG, bắt buộc dùng System.Text.StringBuilder.
- **XML COMMENTS**: **LUÔN LUÔN** phải có XML summary comment (<summary>) cho TẤT CẢ các public methods, classes, properties, VÀ FIELDS.
- **NAMING CONVENTION**: Cấm dùng hậu tố Dto. Tất cả model, dto gom lại theo naming rule chung là {Tên}Model.


## 7. C# Coding Style
- **Namespace**: BẮT BUỘC sử dụng **file-scoped namespace** (VD: amespace UniManage.Shared.Domain.Interfaces;) cho toàn bộ project. KHÔNG dùng block-scoped namespace (amespace { ... }).
